...
export default function AthletCreatePlan() {
    ...
    <ScreenLayout>
      ...
      >
        <DraggableFlatList
                ...
                <View style={{ flexDirection: "row", gap: Spacing.sm, marginBottom: Spacing.sm }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput
                      ...
                      style={{ borderWidth: 1, borderColor: Colors.border, padding: 12, borderRadius: Radius.md, color: Colors.text, backgroundColor: Colors.surface }}
                    />
                  </View>
                  <View style={{ flex: 1 }}>
          ...
          ListFooterComponent={
            error ? <Text style={{ color: Colors.danger, marginTop: Spacing.xs }}>{error}</Text> : <View style={{ height: 1 }} />
          }
            ...
            return (
              <View style={{ marginBottom: Spacing.sm, opacity: isActive ? 0.9 : 1 }}>
                <ExerciseCard
                  ...
                  autoFocusName={item._key === lastAddedKey && expandedKey === item._key && (item.name ?? "").trim() === ""}
                  onToggleExpanded={() =>
                    setExpandedKey((prev) => {
                  ...
                  onDuplicate={() => {
                    const clone: ExerciseDraft = { ...item, _key: `${Date.now()}-${Math.random().toString(16).slice(2)}` };
                    setExercises((prev) => {
      ...
      <View style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: Spacing.md, backgroundColor: Colors.bg, borderTopWidth: 1, borderTopColor: Colors.border }}>
