...
export default function MyTrainingTab() {
  ...
  useFocusEffect(
    useCallback(() => {
      let cancelled = false;
      if (!user || user.role !== "coach") return undefined;
    ...
    <ScreenLayout>
      <View style={{ flex: 1, backgroundColor: Colors.bg }}>
        ...
        >
          ...
          {session ? (
            <Pressable
              onPress={() => router.push({ pathname: "/coach/workoutExecution" as any, params: { workoutPlanId: session.workoutPlanId } })}
              style={({ pressed }) => ({
          ...
          {!showHub ? (
            ...
            <>
              ...
              <View style={{ gap: Spacing.md, marginBottom: Spacing.lg }}>
                {sortedPlans.map((plan) => {
                  ...
                  return (
                    <ScaleCard
                      key={plan.id}
                      style={{ backgroundColor: Colors.card, borderRadius: Radius.lg, padding: Spacing.md, borderWidth: 1, borderColor: Colors.border }}
                    >
                      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                        <View style={{ flex: 1, marginRight: Spacing.sm }}>
              ...
              >
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: Spacing.sm }}>
                  <Text style={{ ...Typography.section, fontWeight: "800" }}>{t("progressSnapshot")}</Text>
