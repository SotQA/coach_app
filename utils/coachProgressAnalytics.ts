...
/**
 * Walk every log in range and collect PRs. A PR is any logged exercise
 * with `isPr === true`. Returns chronological-descending (most recent first).
 */
export function collectPRsInRange(
  logs: WorkoutLog[],
  rangeStartMs: number | null,
  rangeEndMs?: number,
): PRRecord[] {
  const end = rangeEndMs ?? Date.now();
  ...
  for (const log of logs) {
    ...
    for (const ex of log.exercises ?? []) {
      ...
      let repsAtMax = 0;
      for (const s of ex.sets ?? []) {
        if (s.weight != null && Number.isFinite(s.weight) && s.weight > maxWeight) {
          maxWeight = s.weight;
          repsAtMax = s.reps;
        }
      }

      const weightKg = maxWeight > 0 ? maxWeight : null;
      if (weightKg === null) {
        const firstSet = ex.sets?.[0];
        if (firstSet) repsAtMax = firstSet.reps;
      }
  ...
  return records.sort((a, b) => b.completedAtMs - a.completedAtMs);
}

export interface PlateauResult {
  plateau: boolean;
  lastGainMs: number | null;
  weeksWithoutGain: number;
}

/**
 * Given a chronological-ascending series of weekly 1RM values (from
 * `buildWeekly1RMSeries`), detect whether the most recent N weeks
 * contain no new all-time high.
 *
 * Default threshold is 3 weeks. Returns `plateau: true` and the
 * timestamp of the last week that DID hit a new high.
 */
export function detectPlateau(
  series: WeeklyPoint[],
  weeksWithoutGain: number = 3,
): PlateauResult {
  if (series.length === 0) {
    return { plateau: false, lastGainMs: null, weeksWithoutGain: 0 };
  }
  ...
  for (let i = 0; i < series.length; i++) {
    if (series[i].value > runningMax) {
      runningMax = series[i].value;
      lastGainIdx = i;
    }
  }

  const weeksAfterLastGain = series.length - 1 - lastGainIdx;
  const plateau = weeksAfterLastGain >= weeksWithoutGain;
  const lastGainMs = lastGainIdx >= 0 ? series[lastGainIdx].weekStartMs : null;

  return { plateau, lastGainMs, weeksWithoutGain: weeksAfterLastGain };
}

/**
 * Sum session volume per muscle group across the date range.
 * Exercises that map to multiple groups (e.g. deadlift) contribute
 * their full volume to EACH group (compound credit).
 *
 * Pass `rangeStartMs = null` for "all time".
 */
export function volumeByMuscleGroup(
  ...
  for (const log of logs) {
    ...
    for (const ex of log.exercises ?? []) {
      ...
      const vol = sessionVolumeForExercise(ex);
